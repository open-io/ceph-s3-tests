...
def _generate_attributes_decorators(resource, method, function, acl, result):
    ATTRIBUTES_TMPL = """@attr(resource='{resource}')
@attr(method='{method}')
@attr(operation='Test {function} cross account with {acl} acl')
"""
    output = ATTRIBUTES_TMPL.format(
        resource=resource, method=method, function=function, acl=acl
    )
    if result:
        output += "@attr(assertion='success')\n"
    else:
        output += "@attr(assertion='fails')\n"
    return output
...
def _generate_test(
    ...
    if name is None:
        name = function
    output = "\n\n"
    output += _generate_attributes_decorators(
        resource, method, name, acl, result
    )
